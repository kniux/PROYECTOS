/* Versión para LCD de dos líneas*/
...
const int button1   = 3;                   // button1 pin number
const int button2   = 9;                   // button1 pin number
const int button3   = 10;                   // button1 pin number
const int buzzer_pin = 4;                   // Alarms pin number
const byte ROWS = 4;
const byte COLS = 1;
...
void setup() { 
 ...
 // pinMode(button3, INPUT_PULLUP);
  ...
  //Interrupción por cambio de estado de pin
...
void loop() { 
  ...
  if(customKey == '1'){                    // If B1 button is pressed
      i = 0;
      hour   = edit(0, 0, hour);
      minute = edit(3, 0, minute);
      //while(digitalRead(button1));             // Wait until button B1 released
      while(true){
        while(customKey == '2'){           // If button B2 button is pressed
        ...
        if(customKey == '1')               // If button B1 is pressed
          break;
      }
    ...
    if(customKey == '3'){                  // If B3 button is pressed
      while(customKey == '3');             // Wait until button B3 released
    ...
    if(customKey == '2' && digitalRead(buzzer_pin)){         // When button B2 pressed with alarm (Reset and turn OFF the alarm)
...
void alarms_read_display() {                    // Function to read and display alarm1, alarm2 and temperature data
 ...
 // Convert decimal to ASCII
  ...
  while(customKey == '2'){                 // wait press button 2, display alarms
   while(true){
    lcd.setCursor(0, 0);
    lcd.print(alarm1);                            // Display alarm1
    lcd.setCursor(13, 0);
    ...
    if(customKey == '2') break;               // If button B2 is free, return display time/calendar        
   }
 }
}
...
void Blink() {
  char customKey = customKeypad.getKey();
  byte j = 0;
  while( (j < 10 && (customKey != '1')) || (i >= 5)
          && (customKey != '2') && (customKey != '3') || i < 5) {
    j++;
    delay(25);
  }
}
byte edit(byte x, byte y, byte parameter){
  char customKey = customKeypad.getKey();
  char text[3];
  while((customKey == '1') || (customKey == '3'));    // Wait until button B1 is released
  while(true){
    while((customKey == '2')){                           // If button B2 is pressed
      parameter++;
    ...
    Blink();
    if( ((customKey == '1') && i < 5 ) || ((customKey == '3') && i >= 5)){
